 ...
 *
 * 主线程只负责 toJSON（~1-3ms），序列化和文件写入由 Worker 线程完成
 */
class SnapshotManager {
   ...
   */
  _initWorker() {
    var workerPath = path.join(__dirname, 'snapshot-worker.js')
    ...
    this.worker.on('message', msg => {
      if (msg.type === 'saved') {
        ...
        console.error('Snapshot worker error:', msg.message)
        this.saving = false
      }
    })
    this.worker.on('error', err => {
      console.error('Snapshot worker fatal:', err.message)
      this.saving = false
    })
    // Worker 不应阻止进程退出
    this.worker.unref()
  ...
  /**
   * 启动定期快照
   * @param {Function} getStateFn 返回精简状态对象的函数（同步）
   */
  start(getStateFn) {
    ...
    console.log(
      `Snapshot: every ${this.snapshotIntervalSec}s, archive every ${this.archiveEveryN} snapshots, max ${this.maxArchives} archives`
    )
  }
  ...
  /**
   * 执行一次快照（主线程同步获取状态对象，Worker 线程执行序列化和文件写入）
   */
  takeSnapshot(getStateFn) {
    ...
    var latestPath = this.getLatestPath()
    var isArchive = this.snapshotCount % this.archiveEveryN === 0
    var archivePath = isArchive
      ? path.join(this.archiveDir, 'state_' + Date.now() + '.json')
      : null

    // 将状态对象发送给 Worker（structured clone 深拷贝，主线程可安全继续）
  ...
  /**
   * 立即执行一次快照（优雅关闭时调用，同步写入，不经过 Worker）
   */
