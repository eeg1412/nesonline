...
class FrameEncoder {
  ...
  buildPacket(isKeyframe, sendPalette, changedTiles, audioSamples) {
    ...
    const totalSize = headerSize + paletteDataSize + tileCountSize + tileDataSize + audioCountSize + audioDataSize
    ...
    buffer.writeUInt16LE(paletteEntryCount, offset); offset += 2

    // === 调色盘 ===
    if (sendPalette) {
      for (let i = 0; i < this.palette.length; i++) {
        const c = this.palette[i]
        buffer.writeUInt8((c >> 16) & 0xFF, offset); offset += 1 // R
        buffer.writeUInt8((c >> 8) & 0xFF, offset); offset += 1  // G
        buffer.writeUInt8(c & 0xFF, offset); offset += 1          // B
        buffer.writeUInt8(0xFF, offset); offset += 1               // A
      }
    }

    // === 瓦片差量 ===
    buffer.writeUInt16LE(changedTiles.length, offset); offset += 2
    for (const tile of changedTiles) {
      buffer.writeUInt16LE(tile.pos, offset); offset += 2
      buffer.set(tile.data, offset); offset += 64
    }
    ...
    // === 音频 PCM int16 ===
    buffer.writeUInt16LE(audioSamples.length, offset); offset += 2
