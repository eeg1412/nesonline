...
class FrameEncoder {
  constructor(keyframeInterval) {
    ...
    this.frameCount = 0
    this._forceKeyframe = false // 外部请求强制产生关键帧
  ...
  reset() {
    ...
    this.palette = []
    this.colorToIndex = new Map()
    this.prevTileHash.fill(0xffffffff)
  }
  ...
  encodeFrame(pixels, audioSamples) {
    this.frameCount++
    const isKeyframe =
      this._forceKeyframe || this.frameCount % this.keyframeInterval === 1
    this._forceKeyframe = false // 使用后立即重置

    // 关键帧时重置调色盘，保持索引紧凑（防止长期运行后超出 Uint8Array 范围）
    if (isKeyframe) {
      this.palette = []
      this.colorToIndex = new Map()
    }
    ...
    const indexed = new Uint16Array(256 * 240)
